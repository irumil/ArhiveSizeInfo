...
namespace ConsoleHostSizeInfo
{
    class Program
    {
        ...
        private static void InitializeNotifyIcon()
        {
            ...
            _notifyIcon.Icon = Properties.Resources.serverSize;
            _notifyIcon.Visible = true;
            _notifyIcon.Text = @"Это серверная часть для подсчета информации";
        }
        ...
        static void Main()
        {
            ...
            var myServiceHost = new ServiceHost(typeof(SizeService.SizeService));
            myServiceHost.Open();

            LogText("Это серверная часть для подсчета переданной и принятой информации " + Environment.NewLine);
